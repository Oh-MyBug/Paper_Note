...
信号通过移动设备上的扬声器传输，并由麦克风记录，使用相同的采样频率为48kHz。 接收到声音信号后，VSkin首先通过将通带信号向下转换回值基带信号解调 
...
在安卓平台上执行VSkin
 ...
 当手指在手机背面移动6厘米时，VSkin实现了3.59毫米的平均移动距离误差

 对于直径从1厘米到2厘米的不同物体，VSkin是健壮的

 VSkin对于不同的保持风格是健壮的

 VSkin可以可靠地测量速度从2厘米/秒到12厘米/秒的移动距离。

VSkin对距离手机5厘米的干扰动作具有很强的鲁棒性。

 VSkin对背景可听声噪声具有较强的鲁棒性，在噪声干扰下平均移动距离误差为6.22mm。

#### Touch Movement

针对手机背面不同位置，VSkin实现了99.64%的触摸检测率。

VSkin对手机三个不同区域的触摸进行分类，平均准确率达到87.82%

####  Latency and Power Consumption

 VSkin在商用智能手机上的延迟时间为4.83ms

 VSkin在商用智能手机上的耗电量为491.3兆瓦。

#### Discussions

**不同的手机安装**：VSkin可以在不同类型的智能手机上工作

**扬声器和麦克风的位置**： 扬声器和麦克风的位置对VSkin的性能至关重要。

**隐私问题**： 由于VSkin使用麦克风记录声音信号，我们的系统可能会导致潜在的隐私泄漏问题。 一种可能的解决方案是将记录的声音信号保持在操作系统中  只向应用程序提供触摸事件。

### Q&A

- 为什么选择Zadoff-Chu作为发射声音信号
- Summary：本文开发了VSkin，一种能在移动设备表面进行触摸传感的新型手势传感方案。 VSkin的关键洞察力是我们可以用触摸手势来测量  一种高精度的结构传播和空气传播的声学信号
